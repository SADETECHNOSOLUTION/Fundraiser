...
<span className="text-white small:hidden">{formattedPath}</span>
      <div className="hidden med:flex gap-2 small:gap-4 medium-6 sl:gap-10 items-center">
        {headers.map((header) => (
          <NavLink
            key={header.id}
            to={header.path}
      ...
      <div className="flex items-center gap-4 relative">
        <button
          onClick={openFund}
          className="py-2 px-4 hidden border-2 border-white med:flex rounded-full hover:bg-opacity-90 bg-background"
        >
          <p className="text-white font-[600]">Start a fundraiser</p>
        </button>
        <button
          onClick={openBloodRegister}
          className="py-2 px-4 hidden bg-white border-2 border-background med:flex rounded-full"
        >
      ...
      {token ? (
        ...
        >
          Signin
            ...
            {token&& <NavLink to={`/myfundraisers`}><button className="w-full py-2 px-4 text-sm hover:bg-gray-100 rounded-md"> My Fundraiser</button></NavLink>
...
{menu && (
  <div className="fixed inset-0 z-50 bg-white flex flex-col h-screen p-6">
    ...
    <div className="pt-4">
      ...
      {token&& <button className="w-full py-2 px-4 text-red-600 font-semibold rounded-md"> My Fundraiser</button>
